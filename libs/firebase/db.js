...
export const createNoteForFolder = (folderId, note) => db.ref(`folders/${folderId}/notes`).push().set(note);
export const updateFolder = ({ id, title }) => db.ref(`folders/${id}`).set({ title, updatedAt: new Date() });
export const updateNoteForFolder = (folderId, noteId, note) => db.ref(`folders/${folderId}/notes/${noteId}`).set(note);
